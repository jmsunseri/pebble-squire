...
void version_init() {
    ...
    if (version_info_compare(version_info, s_last_launch) != 0) {
        ...
        if (status < 0) {
            SQUIRE_LOG(APP_LOG_LEVEL_WARNING, "Failed to write version info: %d", status);
        } else {
            SQUIRE_LOG(APP_LOG_LEVEL_INFO, "Current version (v%d.%d) stored (previous: v%d.%d)", version_info.major, version_info.minor, s_last_launch.major, s_last_launch.minor);
        }
    } else {
        SQUIRE_LOG(APP_LOG_LEVEL_DEBUG, "Version (v%d.%d) unchanged since last launch.", version_info.major, version_info.minor);
    }
}
...
VersionInfo prv_read_last_launch() {
    ...
    if (status < 0) {
        SQUIRE_LOG(APP_LOG_LEVEL_WARNING, "Failed to read version info: %d", status);
        return (VersionInfo) {
            .major = 0,
            .minor = 0,
        };
