...
GBitmap *bgbitmap_create_with_resource(uint32_t resource_id) {
  ...
  while (true) {
    ...
    if (!memory_pressure_try_free()) {
      SQUIRE_LOG(APP_LOG_LEVEL_ERROR, "Failed to allocate memory: couldn't free enough heap.");
      return NULL;
    }
    int new_heap_size = heap_bytes_free();
    SQUIRE_LOG(APP_LOG_LEVEL_INFO, "Freed %d bytes, heap size is now %d. Retrying gbitmap allocation.", heap_size - new_heap_size, new_heap_size);
  }
  return NULL;
}
...
GDrawCommandImage *bgdraw_command_image_create_with_resource(uint32_t resource_id) {
  ...
  while (true) {
    ...
    int new_heap_size = heap_bytes_free();
    SQUIRE_LOG(APP_LOG_LEVEL_INFO, "Freed %d bytes, heap size is now %d. Retrying gdrawcommandimage allocation.", heap_size - new_heap_size, new_heap_size);
  }
  return NULL;
}
...
GDrawCommandSequence *bgdraw_command_sequence_create_with_resource(uint32_t resource_id) {
  ...
  while (true) {
    ...
    int new_heap_size = heap_bytes_free();
    SQUIRE_LOG(APP_LOG_LEVEL_INFO, "Freed %d bytes, heap size is now %d. Retrying gdrawcommandsequence allocation.", heap_size - new_heap_size, new_heap_size);
  }
  return NULL;
}
