...
void *bmalloc(size_t size) {
  ...
  while (true) {
    ...
    if (heap_bytes_free() > 750) {
      ...
      if (ptr) {
        SQUIRE_LOG(APP_LOG_LEVEL_DEBUG_VERBOSE, "malloc returned %p for caller %p", ptr, saved_lr);
        return ptr;
      }
      SQUIRE_LOG(APP_LOG_LEVEL_WARNING, "Out of memory! Need to allocate %d bytes; %d bytes free.", size, heap_size);
    } else {
      SQUIRE_LOG(APP_LOG_LEVEL_WARNING, "Low memory (%d byte free); trying to free some before allocating %d bytes.", heap_size, size);
    }
    ...
    SQUIRE_LOG(APP_LOG_LEVEL_INFO, "Freed %d bytes, heap size is now %d. Retrying allocation of %d bytes.", heap_size - new_heap_size, new_heap_size, size);
  }
}
