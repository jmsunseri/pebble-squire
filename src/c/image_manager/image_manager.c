...
static void prv_inbox_received(DictionaryIterator *iterator, void *context) {
  ...
  SQUIRE_LOG(APP_LOG_LEVEL_DEBUG, "handling something for image_id: %d", image_id);
...
static void prv_handle_new_image(int image_id, size_t size, DictionaryIterator *iterator) {
  ...
  if (!tuple) {
    SQUIRE_LOG(APP_LOG_LEVEL_WARNING, "Missing IMAGE_WIDTH in message");
    return;
  }
  int16_t width = tuple->value->int32;
  tuple = dict_find(iterator, MESSAGE_KEY_IMAGE_HEIGHT);
  if (!tuple) {
    SQUIRE_LOG(APP_LOG_LEVEL_WARNING, "Missing IMAGE_HEIGHT in message");
    return;
  }
  int16_t height = tuple->value->int32;
  SQUIRE_LOG(APP_LOG_LEVEL_DEBUG, "New image: %d, size: %d, width: %d, height: %d", image_id, size, width, height);
  ManagedImage *image = bmalloc(sizeof(ManagedImage));
  if (!image) {
    SQUIRE_LOG(APP_LOG_LEVEL_WARNING, "Failed to allocate memory for image metadata");
    return;
  }
  image->data = bmalloc(size);
  if (!image->data) {
    SQUIRE_LOG(APP_LOG_LEVEL_WARNING, "Failed to allocate memory for image data");
...
static void prv_handle_image_chunk(int image_id, size_t offset, DictionaryIterator *iterator) {
  SQUIRE_LOG(APP_LOG_LEVEL_DEBUG, "Handling image chunk for image_id: %d, offset: %d", image_id, offset);
  ManagedImage *image = prv_find_image(image_id);
  if (!image) {
    SQUIRE_LOG(APP_LOG_LEVEL_INFO, "Got data for unknown image id %d", image_id);
    return;
  }
  if (!image->data) {
    SQUIRE_LOG(APP_LOG_LEVEL_INFO, "Got data for image we couldn't allocate; discarding.");
    return;
  }
  Tuple *tuple = dict_find(iterator, MESSAGE_KEY_IMAGE_CHUNK_DATA);
  if (!tuple) {
    SQUIRE_LOG(APP_LOG_LEVEL_INFO, "Got data for image id %d with no chunk data!", image_id);
    return;
  }
  if (offset + tuple->length > image->size) {
    SQUIRE_LOG(APP_LOG_LEVEL_INFO, "Image data chunk too large: %d + %d > %d", offset, tuple->length, image->size);
    return;
  }
  SQUIRE_LOG(APP_LOG_LEVEL_DEBUG, "Got %d bytes for image id %d; copying to %p", tuple->length, image_id, image->data + offset);
  memcpy(image->data + offset, tuple->value->data, tuple->length);
}

static void prv_handle_image_complete(int image_id) {
  SQUIRE_LOG(APP_LOG_LEVEL_DEBUG, "Handling image complete for image_id: %d", image_id);
  ManagedImage *image = prv_find_image(image_id);
  if (!image) {
    SQUIRE_LOG(APP_LOG_LEVEL_WARNING, "Got complete for unknown image id %d", image_id);
    return;
  }
  if (!image->data) {
    SQUIRE_LOG(APP_LOG_LEVEL_INFO, "Got complete for image we couldn't allocate; destroying.");
    prv_destroy_image(image);
  ...
  if (!image->bitmap) {
    ...
    if (!image->bitmap) {
      SQUIRE_LOG(APP_LOG_LEVEL_WARNING, "Failed to create bitmap from data");
      return;
    }
    ...
    SQUIRE_LOG(APP_LOG_LEVEL_DEBUG, "Bitmap created: %d x %d, format: %d, bytes_per_row: %d", bounds.size.w, bounds.size.h, format, bytes_per_row);
    if (format == GBitmapFormat2BitPalette) {
      SQUIRE_LOG(APP_LOG_LEVEL_DEBUG, "Palette: %d, %d, %d, %d", palette[0], palette[1], palette[2], palette[3]);
    }
...
static bool prv_handle_memory_pressure(void *context) {
  ...
  SQUIRE_LOG(APP_LOG_LEVEL_WARNING, "Memory pressure! Destroying the oldest image.");
